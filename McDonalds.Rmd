...
df <- df %>%
  mutate (Price = case_when(
    Item == "McFlurry with Reese's Peanut Butter Cups" ~ "$4.29",
    Item == "McFlurry with Oreo Cookies" ~ "$4.29",
    Item == "McFlurry with M&M’s Candies" ~ "$4.29",
    Item == "Shamrock Shake" ~ "$3.69",
    Item == "Chocolate Shake" ~ "$3.69",
    Item == "Strawberry Shake" ~ "$3.69",
    Item == "	Vanilla Shake" ~ "$3.69",
    TRUE                                               ~ Price
    )
  )


#df %>% select(Item,Price)
```
